      ...
      <section id="landing-predict-app" class="section-py bg-body landing-reviews">
        <div class="container">
          <h4 class="text-center mb-3 mt-3">
            <span class="position-relative fw-extrabold z-1">{{ __('predict_title') }} 
                      ...
                      <form id="image-upload">
                        <div class="col-12 px-6">
                          ...
                          <div class="input-group">
                            <label class="input-group-text cursor-pointer" for="imageInput" style="background-color: var(--bs-primary); color: white; border-color: var(--bs-primary);">{{ __('predict_file_select') }}</label>
                            <input type="file" class="form-control d-none" id="imageInput" name="image" accept="image/*" >
                            <input type="text" class="form-control" id="fileText" placeholder="{{ __('predict_no_file') }}" readonly onclick="$('#imageInput').click()" style="cursor: pointer; background-color: var(--bs-body-bg);">
...
@endphp
const __translations = @json($jsTranslations);
  ...
  $(document).ready(function () {
      ...
      $("#imageInput").on("change", function (e) {
          ...
          if (file) {
              ...
              Swal.fire({
                  icon: "success",
                  title: __translations.swal_image_selected,
                  text: file.name,
                  timer: 1000,
                  showConfirmButton: false
              });
              imgMsg.removeClass("d-none").html(`${__translations.swal_selected_image}: <strong>${file.name}</strong>`);
          }
      });
      ...
      $("#predictBtn").on("click", function () {
          ...
          if (model === "0") {
              Swal.fire({
                  icon: "warning",
                  title: __translations.swal_model_not_selected,
                  text: "Lütfen bir model {{ __('predict_instruction_suffix') }}"
              });
              return;
          }
          if (!selectedImage) {
              Swal.fire({
                  icon: "warning",
                  title: __translations.swal_image_missing,
                  text: __translations.swal_image_missing_text
              });
              return;
          }
          ...
          $.ajax({
              ...
              success: function (res) {
                stopSwalTimer(); // Swal kapat + timer durdur
                  Swal.fire({
                    icon: "success",
                    title: __translations.swal_analysis_complete,
                    confirmButtonText: __translations.swal_ok
                  });
                  console.log(res);
                  // Görsel göster
                  if (res.image_url) {
                  ...
                  if (res.probabilities) {
                      ...
                      // Card 1 — Pozitif
                      $("#pozitif").text(positive.toFixed(2) + `% ${__translations.result_positive}`);
                      $("#pozitifSub").text(`ACA: ${aca.toFixed(2)}% — SCC: ${scc.toFixed(2)}%`);

                      // Card 2 — Negatif
                      $("#negatif").text(negative.toFixed(2) + `% ${__translations.result_negative}`);
                      $("#negatifSub").text(`${__translations.result_normal_tissue}: ${normal.toFixed(2)}%`);
                  }
                  else{
                    // Pozitif (%)
                    if (res.positive !== undefined) {
                        $("#pozitif").text(res.positive + `% ${__translations.result_positive}`);
                        $("#pozitifSub").text(`${__translations.result_contains_cancer}`);

                    }
                    // Negatif (%)
                    if (res.negative !== undefined) {
                        $("#negatif").text(res.negative + `% ${__translations.result_negative}`);
                        $("#negatifSub").text(`${__translations.result_no_cancer}`);
...
function startSwalTimer() {
    ...
    Swal.fire({
        title: __translations.swal_analyzing,
        html: `
            <div>
              <div>
